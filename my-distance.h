/**
# Computation of a distance field from a discretised curve or surface2
...
surface2 representations (i.e. surface2s with holes, manifold egdes,
...
/**
The 3D triangulated surface2s are defined using the (binary) [STL
...
coord * input_stl2 (FILE * fp)
{
  ...
  if (fgets (tag, 6, fp) != tag) {
    fprintf (stderr, "input_stl2(): error reading tag\n");
    exit(1);
  }
  rewind (fp);
  if (!strcmp (tag, "solid")) { /* ASCII file */
    fprintf (stderr, "input_stl2(): ASCII STL not implemented yet "
	     "(use binary instead)\n");
    exit(1);
  }
  else { /* binary file */
...
#else // dimension == 3
#  define ND 12
static double vertex_angle2 (coord * p, int type)
{
...
static void update_distance2 (Point point, coord ** i, scalar d)
{
  ...
  if (a->len) {
    // set surface2[] to list, ended with NULL
    coord * p = NULL;
    array_append (a, &p, sizeof(coord *));
    p = (coord *) array_shrink (a);
    assert (sizeof(double) >= sizeof(void *));
    memcpy (&surface2[], &p, sizeof(void *));
...
#elif dimension == 3
    if (q[0].type < 3) {
      coord n = face_normal2 (q[0].v, q[0].type), diff = vecdiff(closest,c);
      int nv = 1;
      orient = sign(vecdot(n,diff));
      for (int i = 1; i < nd && q[i].type < 3; i++)
	if (vecdist2 (closest, *(q[i].v + q[i].type)) < sq(1e-6)) {
	  coord n1 = face_normal2 (q[i].v, q[i].type);
	  foreach_dimension()
	    n.x += n1.x;
	  nv++;
	...
	if (orient !=
	  ...
	  coord n1 = face_normal2 (q[0].v, 3), n2 = face_normal2 (q[1].v, 3), n;
	  foreach_dimension()
	    n.x = n1.x + n2.x;
	  coord diff = vecdiff(closest,c);
	  orient = sign(vecdot(n,diff));
...
static void refine_distance2 (Point point, scalar d)
{
  ...
  if (surface2[] == 0.)
    ...
    coord ** ap = (coord **) double_to_pointer (surface2[]);
    int s = 0;
    foreach_child() {
...
static void restriction_distance2 (Point point, scalar d) {}

static void coarsen_distance2 (Point point, scalar d) {
  scalar surface2 = d.surface2;
  foreach_child()
    free (double_to_pointer (surface2[]));
}

static void delete_distance2 (scalar d) {
  scalar surface2 = d.surface2;
  foreach_level (0)
    free (*((void **)double_to_pointer (surface2[])));
...
void distance2 (scalar d, coord * p)
{
  ...
  if (surface2.i)
    delete_distance2 (d);
  surface2 = new scalar;
  surface2.restriction = no_restriction;
#if TREE
  surface2.prolongation = no_restriction;
  surface2.refine = no_restriction; // handled by refine_distance2()
  d.prolongation = refine_bilinear;
  d.refine = refine_distance2;  
  d.coarsen = coarsen_distance2;
  d.dirty = true;
#endif
  d.surface2 = surface2;
  d.delete = delete_distance2;
  d.restriction = restriction_distance2;
  ...
  foreach_level(0)
    update_distance2 (point, (coord **) p, d);
  free (p);
  
  boundary_level ({d}, 0);
  for (int l = 0; l < depth(); l++) {
    foreach_coarse_level (l)
      refine_distance2 (point, d);
