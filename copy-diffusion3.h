...
trace
mgstats2 diffusion3 (struct Diffusion2 p)
{
...
//  for center of liquid
  bool centerl_flag = false;// true;
  scalar lambda1[];
  if(centerl_flag){
     foreach(){
         r[] = css_test[] * r[];
         lambda1[] = css_test[] * theta_idt[]; //lambda[];
     } 
     return poisson4 (f, r, p.D, lambda1);
  }else{
    return poisson4 (f, r, p.D, lambda);
  }
}
