...
mgstats project_source (struct Project_s q)
{
  ...
  //mgstats mgp = poisson (p, div, alpha,
	//		 tolerance = TOLERANCE*0.001/sq(dt), nrelax = nrelax);
  // relax tolerance to 0.001*0.1
  //mgstats mgp = poisson (p, div, alpha,
	//		 tolerance = TOLERANCE*0.1/sq(dt), nrelax = nrelax);      //tolerance is for (u/deltax*dt), so for tolerance of this eqation is tolerance/(dt^2)
 //  mgstats mgp = poisson (p, div, alpha,
	//		 tolerance = TOLERANCE*0.1/sq(dt), nrelax = nrelax);      //tolerance is for (u/deltax*dt), so for tolerance of this eqation is tolerance/(dt^2)
  //  mgstats mgp = poisson (p, div, alpha,
	// 		 tolerance = TOLERANCE*0.1, nrelax = nrelax);      //tolerance is for (u/deltax*dt), so for tolerance of this eqation is tolerance/(dt^2)


// mgstats mgp = poisson (p, div, alpha,
// 			 tolerance = TOLERANCE*0.001, nrelax = nrelax);

// for lubomir, maxl = 12

// if(maxl==12){
// TOLERANCE = 1e-10; //1e-6;//5e-5;// 6e-6;//4e-6;//1e-7;
// }else if(maxl==10 || maxl==11){
//       TOLERANCE = 1e-6;
// }
// TOLERANCE = 1e-6/dt; 
TOLERANCE = 1e-3/dt; //1e-3/dt;
if(maxl==11){
TOLERANCE = 1e-5/dt; //1e-3/dt;
} 
mgstats mgp = poisson (p, div, alpha,
