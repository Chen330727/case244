			   ...
			   coord n, coord* direction, coord* p, double bc, double * coef)
{
  ...
  else if (fabs(direction1.y) >= fabs(direction1.z))
    return dirichlet_gradient_direction_y (point, s, cs, n, direction1, p1, bc, coef);
  return dirichlet_gradient_direction_z (point, s, cs, n, direction1, p1, bc, coef);
#endif // dimension == 3
  return nodata;
}
...
double embed_flux_direction (Point point, scalar s, face vector mu, coord direction, double bc, double * val)
{
...
void update_tracer (scalar f, face vector uf, face vector flux, double dt)
{
  ...
  foreach() {
    ...
    *f* in time. */
      ...
      foreach_dimension(){
          	// f[] += dt*(flux.x[] - flux.x[1])/Delta; //original advection
          f[] += dt*(flux.x[] - flux.x[1])/(cm[]*Delta); //update tracer
          // f[] += dt*(flux.x[] - flux.x[1])/(max(y,1e-20)*Delta);
      }
      e[] = 0.;
    }
