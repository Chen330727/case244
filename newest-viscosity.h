...
# define lambda ((coord){1., 1. + dt/rho[]*(mu.x[] + mu.x[1]			\
				+ mu.y[] + mu.y[0,1])*sq(cs[])		\
	/(fm.x[] + fm.x[1] + fm.y[] + fm.y[0,1] + SEPS)*2./(cm[] + SEPS)})
// # define lambda ((coord){1., 1. + dt/rho[]*(mu.x[] + mu.x[1]			\
// 				+ mu.y[] + mu.y[0,1])*sq(cs[])		\
// 	/(fm.x[] + fm.x[1] + fm.y[] + fm.y[0,1] + SEPS)/2./(cm[] + SEPS)})
         ...
         double norm_direction=0.0;
            ...
            if(fabs(norm_direction)>1e-20){
                        ...
                        if(!arePerpendicular(direction,direction2)){
                          ...
                          c2 = c2*direction.x;
                          // res.x[] -= dt*(c2 + d2*u.x[])*direction.x;
                        }
                        ...
                        if(!arePerpendicular(direction,direction3)){
                          ...
                          c3 = c3*direction.y;
                          // res.x[] -= dt*(c3 + d3*u.y[])*direction.y;
                        }
            }
            }
// #endif
      ...
      w.x[] = (dt/rho[]*(2.*mu.x[1]*u.x[1] + 2.*mu.x[]*u.x[-1]
			   ...
			   - mu.z[]*(- u.x[0,0,-1] +
				     ...
				     (u.z[-1,0,-1] + u.z[-1,0,0])/4.)
               #endif
			   ) + (r.x[]-dt*(c+c2+c3 + d3*u.y[])/rho[])*sq(Delta))/
    (sq(Delta)*lambda.x +dt*sq(Delta)*(dd+d2)/rho[] + dt/rho[]*(2.*mu.x[1] + 2.*mu.x[]
                                    #if dimension > 1
				      + mu.y[0,1] + mu.y[]
                                    #endif
			            #if dimension > 2
       ...
       double norm_direction=0.0;
          ...
          if(fabs(norm_direction)>1e-20){
                  ...
                  if(!arePerpendicular(direction,direction2)){
                              ...
                              res.x[] -= dt*(c2 + d2*u.x[])/rho[]*direction.x;
                            }
                 ...
                 coord direction3 = (coord){1,0};
                            if(!arePerpendicular(direction,direction3)){
                              ...
                              res.x[] -= dt*(c3 + d3*u.y[])/rho[]*direction.y;
                            }
          }
    }
// #endif
...
mgstats viscosity (struct Viscosity p)
{
  ...
  return mg_solve_viscous ((scalar *){u}, (scalar *){r},
		   residual_viscosity, relax_viscosity, &p, p.nrelax, p.res);
}
