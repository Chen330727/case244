...
static void refine_face_x_axi (Point point, scalar fm)
{
  ...
  if (!is_refined(neighbor(-1))) {
    fff = fine(fs.x,0,0);
    //  if(fabs(fff-1.0)>1e-6)
    //      printf("straing1 x=%g y=%g fff=%g fs.x[]=%g\n",x,y,fff,fs.x[]);
    fine(fm,0,0) = (y - Delta/4. - sig*(1. - fff))*fff;
    fff = fine(fs.x,0,1);
    //  if(fabs(fff-1.0)>1e-6)
    //      printf("straing2 x=%g y=%g fff=%g fs.x[]=%g\n",x,y,fff,fs.x[]);
    fine(fm,0,1) = (y + Delta/4. - sig*(1. - fff))*fff;
  }
  if (!is_refined(neighbor(1)) && neighbor(1).neighbors) {
    fff = fine(fs.x,2,0);
    // if(fabs(fff-1.0)>1e-6)
    //     printf("straing3 x=%g y=%g fff=%g fs.x[]=%g\n",x,y,fff,fs.x[]);
    fine(fm,2,0) = (y - Delta/4. - sig*(1. - fff))*fff;
    fff = fine(fs.x,2,1);
    // if(fabs(fff-1.0)>1e-6)
    //     printf("straing4 x=%g y=%g fff=%g fs.x[]=%g\n",x,y,fff,fs.x[]);
    fine(fm,2,1) = (y + Delta/4. - sig*(1. - fff))*fff;
  }
  fff = fine(fs.x,1,0);
//   if(fabs(fff-1.0)>1e-6)
//         printf("straing5 x=%g y=%g fff=%g fs.x[]=%g\n",x,y,fff,fs.x[]);
  fine(fm,1,0) = (y - Delta/4. - sig*(1. - fff))*fff;
  fff = fine(fs.x,1,1);
//   if(fabs(fff-1.0)>1e-6)
//         printf("straing6 x=%g y=%g fff=%g fs.x[]=%g\n",x,y,fff,fs.x[]);
  fine(fm,1,1) = (y + Delta/4. - sig*(1. - fff))*fff;
#endif // EMBED
}
...
//    if(fabs(fine(fs.y,0,1)-1.0)>1e-6)
//         printf("yyyyyyyyy\n x=%g y=%g fff=%g fs.y[]=%g\n",x,y,fine(fs.y,0,1),fs.y[]);
#endif // EMBED
}
#endif
...
event metric (i = 0) {
  ...
  // printf("metric_mebed ok lalalalal\n");
#endif  
